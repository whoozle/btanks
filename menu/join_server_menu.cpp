...
JoinServerMenu::JoinServerMenu(MainMenu *parent, const int w, const int h) : _parent(parent) {
	...
	int bw, bh, xp, base_x = w / 6;
	_back->getSize(bw, bh);
	add(sdlx::Rect(xp = base_x - 64, h - 96, bw, bh), _back);
	xp += 16 + bw;
	...
	sdlx::Rect list_pos(base_x, 128, (w - 64)/3, h - 256);

	_hosts = new ScrollList(list_pos.w, list_pos.h);
	add(list_pos, _hosts);
	
	sdlx::Rect map_pos(list_pos.x + list_pos.w + 32, 128, (w - 64) / 3, h - 256);
